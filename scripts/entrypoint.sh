...
VLM & Concurrency Options:
  --max-model-length N             Max total tokens (input+output) (env MAX_MODEL_LENGTH, default: ${MAX_MODEL_LENGTH_ENV})
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --max-model-length)
      MAX_MODEL_LENGTH_ENV="${2:-}"; shift 2;;
...
export MAX_MODEL_LENGTH="${MAX_MODEL_LENGTH_ENV}"
...
echo "[entrypoint] VLM Config: MAX_MODEL_LENGTH=$MAX_MODEL_LENGTH MAX_IMAGES_PER_CONVERSATION=$MAX_IMAGES_PER_CONVERSATION IMAGE_TOKEN_BUDGET_PER_IMAGE=$IMAGE_TOKEN_BUDGET_PER_IMAGE SAFETY_MARGIN_TOKENS=$SAFETY_MARGIN_TOKENS"
